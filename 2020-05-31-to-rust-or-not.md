...
Mutability is actually a very useful concept/distinction to make. After all, this has for long been the cornerstone of functional programming. A pleasant surprise was to find out just how many variables never really need to be mutable at all, even in a procedural language. Though variable  is perhaps no longer the best term. It has become an 'assign once' variable. Its only difference from constants is that its value is computed and assigned (once only) at runtime to a location on the stack.
