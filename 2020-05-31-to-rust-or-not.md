...
- Safe multi-threaded execution. Mutation (overwriting) and aliasing (multiple pointers) can never apply to the same memory location at the same time. This is a big one, with most performance gains to be still fully realised and appreciated.
...
Mutability is actually quite a useful concept/distinction to make. A pleasant surprise was to find out just how many variables never need to be mutable at all. Though variable  is perhaps no longer the best term. It has become an 'assign once' variable. Its only difference from constants is that its value is computed and assigned at runtime to a location on the stack.
...
The Rust libraries are called crates and like everything else in Rust, they are remarkable safe, even to import from a vast collection by every Tom Dick and Harry at crates.io.
...
As if this was not complicated enough, some facilities of some crates are available only as their 'special features', which must be known by name and listed in curly brackets, otherwise they will not work. This is annoying because typically somebody might avise you to use a particular crate but almost always forgets to tell you which of its 'special features' you need.
...
When I first started writing this blog, it was going to be all about just Rust's error handling. That is how big a subject it is.  The difficulty is that in its generality, Rust admits to quite a number of diverse styles and philosophies of error handling and almost every programmer has developed their own. 
...
I had gone through several iterations of handling errors in several different ways, mostly in the middle of the road category. The Rust Book was not help much in this regard. Yes, it does argue for error recovery but does not offer much practical help.  Then I came across the `anyhow` crate and I gladly threw all that previous rubbish away. 

Besides recoverable/unrecoverable, I find another classification of errors useful: 

- Hard errors, that is errors usually triggered by your own tests of some wrong/unexpected values, which ought to be reported immediately using the `!bail` macro from `anyhow` (or just `!panic` if you do not need much information in the error report).
...
- Errors returned from some function call, often to an external crate. These will be wrapped up in Result, so from purely technical point of view they need a different handling mechanism. These can be divided further into errors that:
	...
	where the dot follows a Result type returned by some function. It can be either `Ok(some valid value)` or `Error`.
	Anyhow's `.context` thus acts rather like `.expect` but it automatically passes up its generally castable 'anyhow::Error', whereas with .expect you are expected to choose your own error type and pass it up yourself.
