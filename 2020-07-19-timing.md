...
I had permissions problems until I have put `sudo` in front of the invocations of `sleeper` and `rtcwake` within `sleeper`. Which does seem rather superfluous, considering that the script was already invoked with `sudo`.
